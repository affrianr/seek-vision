...
- The user asks how to configure vision-context, get an API key, or switch providers: follow `references/configure.md` and run the commands for them
...
```bash
npx @anthropic-ai/vision-context --version
```

If `vision-context` is missing, run it via `npx @anthropic-ai/vision-context` instead.

Vision Context supports two vision providers. Check what is configured:

```bash
npx @anthropic-ai/vision-context config show
```

- **gemini-api** (recommended, free): needs `GEMINI_API_KEY` env or `npx @anthropic-ai/vision-context config set gemini-api.apiKey <key>` (free key from https://aistudio.google.com). Fast (5-10 seconds per image).
...
```bash
# Basic usage
npx @anthropic-ai/vision-context -i <image-path-or-url>

# Pick a provider explicitly
npx @anthropic-ai/vision-context -i <image> -p gemini-api

# With extra focus
npx @anthropic-ai/vision-context -i <image> --prompt "focus on axes"

# Output to file
npx @anthropic-ai/vision-context -i <image> -o output.json
```
...
1. Run `vision-context` once per image.
